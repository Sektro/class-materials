...
void advanced_handler(int signumber, siginfo_t *info, void*unused){
    ...
    if(Keress_meg == info->si_value.sival_ptr){
        ...
        printf("elírtál valamit! Nem jó adatot küldesz a signallal!\n");
    }
}
...
int main(int argc, char* argv[]){
    ...
    if(gy1){
        ...
        if(gy2){
            ...
            char* chosen = teruletek[rand() % 3];
            write(fo_to_fules[1], chosen, strlen(chosen) + 1);
            
            /*to_send.type = FO_TO_TAPSI;
            strcpy(to_send.buf, teruletek[rand() % 7]);
            msgsnd(msgQ, &to_send, sizeof(to_send) - sizeof(long), 0);*/
            chosen = teruletek[rand() % 4 + 3];
            write(fo_to_tapsi[1], chosen, strlen(chosen) + 1);
            
            /*struct szam_msg to_receive;
            msgrcv(msgQ, &to_receive, sizeof(to_receive) - sizeof(long), TAPSI_TO_FO, 0);*/
            int tapsi_szamlalt;
            read(tapsi_to_fo[0], &tapsi_szamlalt, sizeof(tapsi_szamlalt));
            int tapsi_helyen_tavaly = rand() % 51 + 50;
            int gyarapodas_tapsi = tapsi_szamlalt > tapsi_helyen_tavaly;
            pt("Tapsi ennyit számolt: %d, tehát %s történt", tapsi_szamlalt, (gyarapodas_tapsi ? "gyarapodás" : "fogyás"));

            //msgrcv(msgQ, &to_receive, sizeof(to_receive) - sizeof(long), FULES_TO_FO, 0);
            int fules_szamlalt;
            read(fules_to_fo[0], &fules_szamlalt, sizeof(fules_szamlalt));
            int fules_helyen_tavaly = rand() % 51 + 50;
            int gyarapodas_fules = fules_szamlalt > fules_helyen_tavaly;
            pt("Füles ennyit számolt: %d, tehát %s történt", fules_szamlalt, (gyarapodas_fules ? "gyarapodás" : "fogyás"));
            ...
            struct sembuf muvelet = {.sem_num = 0, .sem_op = -(!gyarapodas_tapsi + !gyarapodas_fules), .sem_flg = 0};
            semop(sem, &muvelet, 1);
            if(!gyarapodas_tapsi){
                //msgrcv(msgQ, &to_receive, sizeof(to_receive) - sizeof(long), TAPSI_TO_FO, 0);
                //read(tapsi_to_fo[0], &tapsi_szamlalt, sizeof(tapsi_szamlalt));
                fules_szamlalt = ((int*)shared)[0];
                pt("Tapsi újraszámolt, ennyi lett a végeredmény: %d", tapsi_szamlalt);
                //tapsi_szamlalt = to_receive.nepszam;
            }
            if(!gyarapodas_fules){
                //msgrcv(msgQ, &to_receive, sizeof(to_receive) - sizeof(long), FULES_TO_FO, 0);
                //read(fules_to_fo[0], &fules_szamlalt, sizeof(fules_szamlalt));
                fules_szamlalt = ((int*)shared)[1];
                pt("Füles újraszámolt, ennyi lett a végeredmény: %d", fules_szamlalt);
            ...
            if(tapsi_helyen_tavaly < tapsi_szamlalt){
                pt("Tapsi gyarapodást tapasztalt.");
            }else{
                pt("Tapsi népességfogyást tapasztalt.");
            }

            //pt("fules_helyen_tavaly (%d) fules_szamlalt (%d)", fules_helyen_tavaly, fules_szamlalt);
            if(fules_helyen_tavaly < fules_szamlalt){
                pt("Füles gyarapodást tapasztalt.");
            }else{
                pt("Füles népességfogyást tapasztalt.");
            }
            ...
            read(fo_to_tapsi[0], buf, sizeof(buf));
            pt("Itt számolok: %s", buf);

            /*struct szam_msg to_send = {.type = TAPSI_TO_FO, .nepszam = rand() % 51 + 50};
            msgsnd(msgQ, &to_send, sizeof(to_send) - sizeof(long), 0);*/
            int nepszam = rand() % 41 + 60;
            write(tapsi_to_fo[1], &nepszam, sizeof(nepszam));
        ...
        read(fo_to_fules[0], buf, sizeof(buf));
        pt("Itt számolok: %s", buf);

        /*struct szam_msg to_send = {.type = FULES_TO_FO, .nepszam = rand() % 51 + 50};
        msgsnd(msgQ, &to_send, sizeof(to_send) - sizeof(long), 0);*/
        int nepszam = rand() % 41 + 60;
        write(fules_to_fo[1], &nepszam, sizeof(nepszam));
