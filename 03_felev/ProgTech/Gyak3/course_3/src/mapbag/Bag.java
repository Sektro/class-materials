...
public class Bag {
    ...
    public Bag intersection(Bag bag) {
        Bag result = new Bag();
        for (Map.Entry<String, Integer> entry1 : bag.data.entrySet()) {
            for (Map.Entry<String, Integer> entry2 : this.data.entrySet()) {
                if (entry1.getKey().equals(entry2.getKey())) {
                    if (entry1.getValue() > entry2.getValue()) {
                        ...
                        result.add(entry1.getKey(), entry1.getValue());
                    }
                }
            }
        }
    ...
    public Bag difference(Bag bag) {
        Bag result = this;
        for (Map.Entry<String, Integer> e : bag.data.entrySet()) {
            if (result.data.containsKey(e.getKey())) {
