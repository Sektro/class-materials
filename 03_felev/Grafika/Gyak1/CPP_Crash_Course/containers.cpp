...
void containers()
{
	...
	emptyVector.push_back(3); //List<T>.Add()
	...
	int* p = vectorWithValues.data(); //vectorWithValues: a vektor elsõ eleme, data(): megadja a pointerét 
	int* p = &vectorWithValues[0]; // Ez is mûködik
	//push_backek után változhat az elsõ elem helye!
	*p = 10;
...
void printVector(const std::vector<int>& vec)
{
	...
	for (const int& v : vec)
	//for (int& v : vec) nem fordul (nem const)
	//for (int v : vec) fordul (másolat készül)
	{
		std::cout << v << " ";
	}
	std::cout << std::endl;
